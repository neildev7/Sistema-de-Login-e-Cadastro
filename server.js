...
// Funções de renderização corrigidas e unificadas
const renderLogin = (res, data = {}, statusCode = 200) => {
...
const renderCadastro = (res, data = {}, statusCode = 200) => {
  return res.status(statusCode).render("cadastro", {
    ...data,
    statusCode: statusCode >= 400 ? statusCode : null,
    minPasswordLength: MIN_PASSWORD_LENGTH,
    maxPasswordLength: MAX_PASSWORD_LENGTH,
...
const requireAuth = (req, res, next) => {
  if (!req.usuarioLogado) {
    if (req.method === "GET") {
      // STATUS 401: Renderiza o login se for GET
      return renderLogin(res, { error: "Faça login para acessar esta página." }, 401);
    }
    // STATUS 401: Retorna JSON se for API/Post/etc
...
app.post("/cadastro", async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      return renderCadastro(res, { error: "Esse email já está cadastrado.", nome, email }, 409); // 409 Conflict
    }
    ...
    return renderCadastro(res, { error: "Erro interno ao cadastrar. Tente novamente.", nome, email }, 500);
  }
});
...
app.post("/login", loginLimiter, async (req, res) => {
  ...
  try {
    ...
    return renderLogin(res, { error: "Erro interno ao realizar login. Tente novamente.", email }, 500);
  }
});
...
app.put("/recuperar-senha", passwordRecoveryLimiter, async (req, res) => {
  ...
  if (!tokenData || tokenData.expiresAt < Date.now()) {
    ...
    // STATUS 403: Proibido devido ao token expirado/ausente
  ...
  if (!safeCompare(tokenHash, tokenData.tokenHash)) {
    // STATUS 403: Proibido devido ao hash inválido
    return res.status(403).json({ status: 403, message: "Token inválido ou expirado." });
  }
...
app.use((req, res) => {
  return renderLogin(res, { error: "Página não encontrada." }, 404);
});
