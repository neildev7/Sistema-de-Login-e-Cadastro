...
const isDatabaseConnectionError = (err) => DATABASE_CONNECTION_ERROR_CODES.has(err?.code);

const logDatabaseError = (context, err) => {
  if (isDatabaseConnectionError(err)) {
    console.warn(`${context}: banco de dados indisponível (${err.code}). Verifique o MySQL e o arquivo .env.`);
    return;
  }
...
app.get("/home", requireAuth, async (req, res) => {
  try {
    ...
    logDatabaseError("Erro ao buscar usuário", err);
    return renderLogin(res, { error: "Não foi possível acessar o banco de dados. Verifique a conexão e tente novamente." }, 503);
  }
});
...
app.post("/cadastro", async (req, res) => {
  ...
  try {
    ...
    logDatabaseError("Erro ao cadastrar usuário", err);
    return renderCadastro(res, { error: "Erro interno ao cadastrar. Verifique o banco de dados e tente novamente.", nome, email }, isDatabaseConnectionError(err) ? 503 : 500);
  }
});
...
app.post("/login", loginLimiter, async (req, res) => {
  ...
  try {
    ...
    logDatabaseError("Erro ao realizar login", err);
    return renderLogin(res, { error: "Erro interno ao realizar login. Verifique o banco de dados e tente novamente.", email }, isDatabaseConnectionError(err) ? 503 : 500);
  }
});
...
app.put("/usuario/nome", requireAuth, async (req, res) => {
  ...
  try {
    ...
    logDatabaseError("Erro ao atualizar nome", err);
    const statusCode = isDatabaseConnectionError(err) ? 503 : 500;
    res.status(statusCode).json({ status: statusCode, message: "Erro interno ao atualizar nome." });
  }
});
...
app.put("/usuario/senha", requireAuth, async (req, res) => {
  ...
  try {
    ...
    logDatabaseError("Erro ao atualizar senha", err);
    const statusCode = isDatabaseConnectionError(err) ? 503 : 500;
    res.status(statusCode).json({ status: statusCode, message: "Erro interno ao atualizar senha." });
  }
});
...
app.post("/recuperar-senha", passwordRecoveryLimiter, async (req, res) => {
  ...
  try {
    ...
    logDatabaseError("Erro ao solicitar recuperação de senha", err);
    const statusCode = isDatabaseConnectionError(err) ? 503 : 500;
    return res.status(statusCode).json({ status: statusCode, message: "Erro interno ao solicitar recuperação de senha." });
  }
});
...
app.put("/recuperar-senha", passwordRecoveryLimiter, async (req, res) => {
  ...
  try {
    ...
    logDatabaseError("Erro ao recuperar senha", err);
    const statusCode = isDatabaseConnectionError(err) ? 503 : 500;
    res.status(statusCode).json({ status: statusCode, message: "Erro interno ao atualizar senha." });
  }
});
...
app.delete("/usuario", requireAuth, async (req, res) => {
  try {
    ...
    logDatabaseError("Erro ao deletar usuário", err);
    const statusCode = isDatabaseConnectionError(err) ? 503 : 500;
    res.status(statusCode).json({ status: statusCode, message: "Erro ao excluir conta." });
  }
});
...
(async () => {
  try {
    ...
    logDatabaseError("Aviso ao conectar ao banco de dados", err);
  }
})();
